  ...
  return (
    <header className="header-island">
      ...
      <div className="header-actions">
        <a href="https://discord.aiobsessive.com" target="_blank" rel="noopener noreferrer" className="discord-link" title="Discord Support">
      ...
      <div className="lang-selector-wrapper">
        <div className={`custom-select ${isLangOpen ? 'open' : ''}`}>
          <div className="select-trigger" onClick={() => setIsLangOpen(!isLangOpen)}>
            <img
              src={`https://flagcdn.com/w40/${lang === 'en' ? 'us' : lang === 'ar' ? 'sa' : lang === 'ja' ? 'jp' : lang}.png`}
              alt={lang}
              className="flag-img"
            />
            ...
            <div className="select-options">
              {['tr', 'en', 'es', 'fr', 'de', 'pt', 'it', 'ru', 'ja', 'ar', 'id'].map((l) => (
                <div key={l} className="option" onClick={() => handleLangChange(l)}>
                  <img
                    src={`https://flagcdn.com/w40/${l === 'en' ? 'us' : l === 'ar' ? 'sa' : l === 'ja' ? 'jp' : l}.png`}
                    alt={l}
                    className="flag-img"
                  /> {l.toUpperCase()}
                </div>
